...
export const description = "Creates or reuses a Penpot design-system board.";

const DESIGN_SYSTEM_TEXT = `# Web Steps — Mini Design System
...
const PENPOT_AGENT_INSTRUCTIONS = [
  ...
  "",
  "When tool names are adapted by Flue, call the matching MCP tools, for example mcp__penpot__createBoard.",
  "Do not use penpot.createShape. It does not exist.",
  "Work in small verified chunks rather than one huge script.",
].join("\n");

function buildPenpotPrompt(input: { designSystemText: string }) {
  return [
...
export async function run({
  ...
  try {
    ...
    const prompt = buildPenpotPrompt({
      designSystemText: payload.designSystemText ?? DESIGN_SYSTEM_TEXT,
    });
