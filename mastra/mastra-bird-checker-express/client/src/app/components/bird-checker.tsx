...
import { getImage, type Image, type ImageQuery } from "@/lib/api";
...
export const BirdChecker = () => {
  ...
  const {
    ...
    queryFn: () => getImage({ query: query as ImageQuery }),
    enabled: !!query,
  });

  const image = result?.ok ? result.data : null;
          ...
          <div className="flex md:flex-col flex-col-reverse gap-4">
            <div>
              ...
              >
                ...
                {status === "idle" ? null : status === "success" &&
                  image?.urls?.regular ? (
                  <ImageComponent
