...
import { promptOpenai } from "@/lib/api";
...
export const BirdCheckerResponse = ({
  ...
  const mutation = useMutation({
    mutationFn: () => promptOpenai({ imageUrl: imageUrl || "" }),
    onSuccess: (data) => {
  ...
  const metadata = mutation.data?.ok
    ? { ...mutation.data.data, bird: mutation.data.data.bird ? "yes" : "no" }
    : null;
  const metadataStatus: Status = mutation.isPending
    ? "loading"
    : mutation.isError
    ? "error"
    : metadata
