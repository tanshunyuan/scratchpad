...
import { getRandomImage, type Image, type ImageResponse } from "./system-tools";
import { z } from "zod";
...
export const promptOpenai = async ({
  ...
  try {
    const birdAgent = mastra.getAgent("birdCheckerAgent");
    ...
    console.log("openai claude response====", JSON.stringify(object, null, 2));

    return { ok: true, data: object as BirdResponse };
  } catch (err) {
    console.error("Error prompting openai:", err);
